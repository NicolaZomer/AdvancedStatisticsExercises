...
p_custom <- function(t){
  ...
  for (i in seq_along(t)){
    integrals[i] <- integrate(d_custom, lower=-Inf, upper = t[i], rel.tol = 1e-12)$value
  }
  return(integrals)
  }
...
q_custom <- function(y){
  ...
  for (i in seq_along(y)){
    ...
    else output[i] <- inverse(p_custom)(y[i])
  }
  return(output)
}

# Generate random numbers from the distribution
r_custom <- function(n, seed=1){
  set.seed(seed)
  q_custom(runif(n))
}
...
plot_pdf <- ggplot()+
  geom_col(aes(x=factor(time), y=d_custom(time)),  fill='gold', colour="black") +
...
plot_cdf <- ggplot()+
  geom_col(aes(x=factor(time), y=p_custom(time)),  fill='gold', colour="black") +
...
ggplot() + 
  geom_histogram(aes(x=r_custom(10000), y=..density..),  binwidth = 0.05, center = 0.05, fill='gold', colour="black") +
  labs(title="Sampling results from the defined pdf", 
       x='Time',
       y='Count')+
...
cat('Probability that the student will finish the test in more than 75 minutes:', 1-p_custom(time_1), '%')
