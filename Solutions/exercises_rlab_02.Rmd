...
# CDF
p_custom_1 <- function(t){
  integrals = numeric(length(t))
  for (i in seq_along(t)){
    integrals[i] <- integrate(d_custom_1, lower=-Inf, upper = t[i], rel.tol = 1e-12)$value
  }
  return(integrals)
  }
...
# Quantile function
q_custom_1 <- function(y){
  output <- numeric(length(y))
  for (i in seq_along(y)){
    if (y[i]==1) output[i]<-2       # boundary value
    else if (y[i]==0) output[i]<-1  # boundary value
    else output[i] <- inverse(p_custom_1)(y[i])
  }
  return(output)
}
...
# CDF
p_custom_2 <- function(x){
  integrals = numeric(length(x))
  for (i in seq_along(x)){
    integrals[i] <- integrate(d_custom_2, lower=-Inf, upper = x[i], rel.tol=1e-7)$value
  }
  return(integrals)
  }

# Quantile function
q_custom_2 <- function(y){
  output <- numeric(length(y))
  for (i in seq_along(y)){
    if (y[i]==1) output[i]<-2       # boundary value
    else if (y[i]==0) output[i]<-1  # boundary value
    else output[i] <- inverse(p_custom_2)(y[i])
  }
  return(output)
}
